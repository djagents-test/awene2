...
function statusLabel(registrationStatus?: string, eventPast?: boolean) {
  if (eventPast) return { label: "Terminé", bg: "#F8EEF5", color: "#4B1F7A" };
  if (registrationStatus === "open") return { label: "Inscriptions ouvertes", bg: "#ECFBF4", color: "#1A8A5A" };
  if (registrationStatus === "full") return { label: "Complet", bg: "#FEF3E8", color: "#F68B2C" };
  return { label: "Inscriptions fermées", bg: "#F8EEF5", color: "#4B1F7A" };
}

function formatLabel(locationType?: string) {
  if (locationType === "online") return "En ligne";
  if (locationType === "hybrid") return "Hybride";
  if (locationType === "in_person") return "Présentiel";
  return null;
}
...
export default async function EvenementSlugPage({ params }: Props) {
  const { slug } = await params;
  const event = await getEventBySlug(slug, "fr");
  if (!event) notFound();
    ...
    <main className="min-h-screen bg-[#FCFAF8] pb-24 pt-36">
      <Container>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-5">
              ...
              {event.locationType === "online" ? (
                <InfoItem icon={<MonitorIcon />} label="Format" value="En ligne" />
              ...
              {typeof event.availableSeats === "number" ? (
                <InfoItem
                  icon={<UsersIcon />}
                  label="Places"
                  value={event.availableSeats === 0 ? "Complet" : `${event.availableSeats} disponibles`}
        ...
        {past ? (
          <div className="mt-16 space-y-10">
            ...
            {satisfactionForm?.active ? (
              <section id="satisfaction" className="max-w-4xl rounded-[2rem] border bg-white p-8 shadow-sm" style={{ borderColor: "#E8DFF0" }}>
                <h2 className="mb-5 text-2xl font-bold md:text-3xl" style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>
                  Votre avis nous aide à améliorer les prochains événements.
                </h2>
          ...
          <section id="inscription" className="mt-16 max-w-4xl">
            <div
              className="rounded-[2rem] border bg-white p-8 shadow-sm"
              style={{ borderColor: "#E8DFF0" }}
            >
              ...
              {registrationOpen ? (
                ...
                <p className="text-base font-medium" style={{ color: "#F68B2C", fontFamily: "var(--font-inter)" }}>
                  Cet événement est complet. Inscrivez-vous à la newsletter pour être informée des prochains événements.
                </p>
              ) : (
        ...
        {/* ── Share section ── */}
        <section id="partager" className="mt-16 max-w-4xl">
          <h3
          ...
          <EventShareButtons title={event.title} url={`https://www.awene.net/fr/evenements/${slug}`} locale="fr" />
        </section>
