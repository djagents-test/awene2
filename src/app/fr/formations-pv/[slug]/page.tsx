...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      pv.shortSummary ||
      "Retour sur cette formation AWENE, les enseignements clés, les ressources partagées et les retours des participantes.",
    openGraph: {
      title: `${pv.pvTitle} | AWENE`,
      description:
        pv.shortSummary ||
...
export default async function FormationPvDetailPage({ params }: Props) {
  const { slug } = await params;
  const pv = await getFormationPvBySlug(slug);
  if (!pv) notFound();

  const relatedFormations = (await getFormationPvs({ language: "fr", limit: 6 }))
  ...
  const statItems = [
    ...
    { label: "Note moyenne", value: pv.averageRating ? `${pv.averageRating}/5` : "—" },
    { label: "Recommandation", value: pv.recommendationRate ? `${pv.recommendationRate}%` : "—" },
  ];

  const resources = [
    { label: "Présentation PDF", url: pv.presentationPdf },
    { label: "Workbook PDF", url: pv.workbookPdf },
    { label: "Exemple de certificat", url: pv.certificateSamplePdf },
    { label: "Rapport de formation", url: pv.formationReportPdf },
        ...
        <section className="grid gap-8 rounded-[2.5rem] border bg-white p-6 shadow-[0_20px_70px_rgba(75,31,122,0.08)] md:grid-cols-[1.1fr_0.9fr] md:p-10" style={{ borderColor: "#E8DFF0" }}>
          <div>
            ...
            <p className="max-w-3xl text-base leading-8 md:text-lg" style={{ color: "#6E6478" }}>
              {pv.shortSummary || "Une étude de cas AWENE sur la formation délivrée, ses enseignements, les ressources partagées et l’impact pédagogique observé."}
            </p>
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              <div className="rounded-[1.25rem] border p-4" style={{ borderColor: "#EFE7F6" }}>
                ...
                <p>{pv.formationDate || "À confirmer"}</p>
              </div>
              <div className="rounded-[1.25rem] border p-4" style={{ borderColor: "#EFE7F6" }}>
              ...
              <div className="rounded-[1.25rem] border p-4" style={{ borderColor: "#EFE7F6" }}>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: "#9B8EA8" }}>Public</p>
                <p>{pv.audienceLabel || "À confirmer"}</p>
              </div>
              <div className="rounded-[1.25rem] border p-4" style={{ borderColor: "#EFE7F6" }}>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: "#9B8EA8" }}>Durée</p>
                <p>{pv.duration || "À confirmer"}</p>
              </div>
            </div>
          </div>
        ...
        <section className="mt-10 grid gap-4 rounded-[2rem] border bg-white p-6 md:grid-cols-4 md:p-8" style={{ borderColor: "#E8DFF0" }}>
          {statItems.map((item) => (
            <div key={item.label} className="rounded-[1.35rem] border p-4" style={{ borderColor: "#EFE7F6", background: "#FFFEFD" }}>
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: "#9B8EA8" }}>
                {item.label}
              </p>
              <p className="text-2xl font-bold" style={{ color: "#2E2438", fontFamily: "var(--font-playfair)" }}>
                {item.value}
              </p>
            </div>
          ))}
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="space-y-10">
            <section className="rounded-[2rem] border bg-white p-6 md:p-8" style={{ borderColor: "#E8DFF0" }}>
              ...
              <div className="prose max-w-none [&_p]:leading-8" dangerouslySetInnerHTML={{ __html: pv.fullRecap || "<p>Le récap complet sera bientôt disponible.</p>" }} />
            </section>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {resources.map((resource) => (
                    <a
                      key={resource.label}
                      href={resource.url}
                      className="rounded-[1.25rem] border px-5 py-4 font-semibold"
                      style={{ borderColor: "#EFE7F6", color: "#6F3FD6" }}
                    >
                      {resource.label}
                    </a>
                  ))}
          ...
          <aside className="space-y-6">
            {linkedFormation ? (
              <section className="rounded-[2rem] border bg-white p-6" style={{ borderColor: "#E8DFF0" }}>
                ...
                <Link href={`/fr/formations/${linkedFormation.slug}`} className="font-semibold" style={{ color: "#6F3FD6" }}>
                  Voir la formation
                </Link>
              </section>
            ) : null}
            ...
            {relatedFormations.length ? (
              <section className="rounded-[2rem] border bg-white p-6" style={{ borderColor: "#E8DFF0" }}>
                ...
                <div className="space-y-4">
                  {relatedFormations.map((item) => (
                    <Link key={item.id} href={`/fr/formations-pv/${item.slug}`} className="block rounded-[1.25rem] border p-4" style={{ borderColor: "#EFE7F6" }}>
            ...
            <section className="rounded-[2rem] border p-6" style={{ borderColor: "#E8DFF0", background: "linear-gradient(135deg, rgba(243,236,251,0.95) 0%, rgba(255,247,239,0.95) 100%)" }}>
              ...
              <h2 className="mb-3 text-2xl font-bold" style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>
                Découvrir nos prochaines formations
              </h2>
              <div className="flex flex-col gap-3">
                <Link href="/fr/formations" className="inline-flex min-h-12 items-center justify-center rounded-full px-5 py-3 text-sm font-semibold text-white" style={{ background: "linear-gradient(135deg, #F68B2C 0%, #E67520 100%)" }}>
                  Voir les formations
                </Link>
                <Link href="/fr/contact?subject=Réserver%20un%20appel" className="inline-flex min-h-12 items-center justify-center rounded-full border px-5 py-3 text-sm font-semibold" style={{ borderColor: "#DCCBFF", color: "#2E2438" }}>
                  Réserver un appel
                </Link>
              </div>
            </section>
