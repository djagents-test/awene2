...
function registrationStatusLabel(status?: string) {
  ...
  if (status === "closed") return "Inscriptions fermées";
  return "Ouvert";
}

export default async function EvenementDetailPage({ searchParams }: Props) {
  const { slug } = await searchParams;
  if (!slug) notFound();
  ...
  const publicUrl = `https://www.awene.net/fr/evenements/${event.slug}`;

  const infoItems = [
    { label: "Date", value: event.date || "À confirmer" },
    { label: "Horaire", value: event.time || "À confirmer" },
    { label: "Lieu", value: event.location || "À confirmer" },
    { label: "Type", value: event.type || "Événement" },
    {
      label: "Places",
      value:
    ...
    { label: "Statut", value: past ? "Terminé" : registrationStatusLabel(event.registrationStatus) },
  ];
          ...
          <div className="relative z-10 grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_24rem] lg:items-start">
            <div>
              <p className="mb-5 inline-flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.25em]" style={{ color: "#F68B2C", fontFamily: "var(--font-inter)" }}>
                ...
                {past ? "Récap événement" : "Événement"}
              </p>
              ...
              <p className="max-w-3xl text-base leading-8 md:text-lg" style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>
                {event.shortDescription || "Découvrez un temps d’échange AWENE conçu pour comprendre, réguler et avancer avec des repères concrets."}
              </p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                ...
                >
                  {past ? "En savoir plus" : "Je m’inscris"}
                </a>
                <a
                  href="#share-event"
            ...
            <aside className="rounded-[2rem] border p-5 shadow-[0_16px_40px_rgba(75,31,122,0.07)] md:p-6" style={{ borderColor: "#E8DFF0", background: "rgba(255,255,255,0.94)" }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {infoItems.map((item) => (
                  <div key={item.label} className="rounded-[1.35rem] border p-4" style={{ borderColor: "#EFE7F6", background: item.label === "Statut" ? "#F8F4FC" : "#FFFEFD" }}>
          ...
          <div className="mt-12 space-y-10">
            {pv?.fullDescription ? (
              <section className="rounded-[2rem] border bg-white p-6 md:p-8" style={{ borderColor: "#E8DFF0" }}>
                <h2 className="mb-4 text-3xl font-bold" style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>Retour sur l’événement</h2>
            ...
            {pv?.keyTakeaways ? (
              <section className="rounded-[2rem] border bg-white p-6 md:p-8" style={{ borderColor: "#E8DFF0" }}>
                <h2 className="mb-4 text-3xl font-bold" style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>Points clés</h2>
                <p className="max-w-[50rem] text-base leading-8" style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>{pv.keyTakeaways}</p>
              </section>
            ) : null}
            {pv?.gallery?.length ? (
              <section className="rounded-[2rem] border bg-white p-6 md:p-8" style={{ borderColor: "#E8DFF0" }}>
                <h2 className="mb-5 text-3xl font-bold" style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>Galerie</h2>
            ...
            <section id="share-event" className="rounded-[2rem] border bg-white p-6 md:p-8" style={{ borderColor: "#E8DFF0" }}>
              ...
              <EventShareButtons locale="fr" title={event.title} url={publicUrl} />
            </section>
            <div className="flex flex-wrap gap-4">
              ...
              <Link href="/fr/evenements" className="inline-flex items-center justify-center rounded-full border px-7 py-3.5 text-sm font-semibold" style={{ borderColor: "#E8DFF0", color: "#2E2438", fontFamily: "var(--font-inter)" }}>
            ...
            {satisfactionForm?.active ? (
              <section id="satisfaction" className="rounded-[2rem] border bg-white p-6 md:p-8" style={{ borderColor: "#E8DFF0" }}>
                <h2 className="mb-4 text-3xl font-bold" style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>Votre avis nous aide à améliorer les prochains événements.</h2>
                <SatisfactionForm locale="fr" eventId={event.id} form={satisfactionForm} />
              </section>
            ) : null}
          </div>
        ) : (
            ...
            <section id="registration" className="rounded-[2rem] border bg-white p-6 shadow-[0_18px_48px_rgba(75,31,122,0.06)] md:p-8 lg:p-10" style={{ borderColor: "#E8DFF0" }}>
              ...
              <div className="mb-7 rounded-[1.5rem] border px-5 py-4" style={{ borderColor: "#F3E3CF", background: "#FFF7EF" }}>
                <p className="text-sm leading-7 md:text-base" style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>
                  Réservez votre place pour cet événement AWENE. Les informations partagées ici servent uniquement à traiter votre inscription.
                </p>
              </div>
              {event.registrationStatus === "open" ? (
                <EventRegistrationForm locale="fr" eventId={event.id} eventTitle={event.title} />
              ) : (
                <div className="rounded-[1.5rem] border px-5 py-4" style={{ borderColor: "#E8DFF0", background: "#FCFAF8" }}>
                  <p style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>
                    {event.registrationStatus === "full" ? "Complet" : "Inscriptions fermées"}
                  </p>
                </div>
              )}
            </section>
