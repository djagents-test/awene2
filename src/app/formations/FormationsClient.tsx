...
function trackFormationCta(formation: CmsFormation, ctaText: string) {
  ...
  window.dispatchEvent(
    new CustomEvent("awene:formation_cta", {
      detail: payload,
    }),
  );

  // Future CRM/analytics integration point: replace or complement this with
  // the site CRM API once a dedicated endpoint is available.
  const dataLayer = (window as Window & { dataLayer?: unknown[] }).dataLayer;
  dataLayer?.push({
    event: "formation_cta_click",
    ...payload,
  });
}
...
function ctaForFormation(formation: CmsFormation) {
  ...
  if (formation.status === "past") {
    return {
      text: "Voir les prochaines formations",
      href: "/formations",
      disabled: false,
      external: false,
    };
...
function FormationCard({ formation }: { formation: CmsFormation }) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <dt className="font-semibold" style={{ color: "#4B1F7A" }}>
            Date
          </dt>
        ...
        {typeof formation.remainingSeats === "number" && (
          <div>
            ...
            <dd>{formation.remainingSeats} restantes</dd>
          </div>
        )}
      </dl>
...
export function FormationFilters({ formations }: { formations: CmsFormation[] }) {
  ...
  const filteredFormations = useMemo(
    () =>
      formations.filter((formation) => {
        if (filterState.format !== "all" && formation.format !== filterState.format) {
          return false;
        }

        if (filterState.audience !== "all" && formation.audience !== filterState.audience) {
          return false;
        }

        if (filterState.language !== "all" && formation.language !== filterState.language) {
          return false;
        }

        if (filterState.status !== "all" && formation.status !== filterState.status) {
          return false;
        }

        if (filterState.date === "upcoming" && formation.status !== "upcoming") {
          return false;
        }

        if (filterState.date === "month" && !isThisMonth(formation.startDate)) {
          return false;
        }
        ...
        >
          <div>
            ...
            <p
              ...
              style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}
            >
              Les prochaines dates seront publiées ici dès leur confirmation. Vous
              pouvez aussi demander une session sur mesure.
            </p>
            <a
          ...
          <div className="relative aspect-[1.15/1] w-full overflow-hidden rounded-[1.75rem]">
            <Image
              ...
              alt="Portrait artistique en noir et blanc d une femme reflechissant"
              fill
...
export function FormationsFAQ({ faqs }: { faqs: Faq[] }) {
