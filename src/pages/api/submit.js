...
const brevoListId = Number.parseInt(import.meta.env.BREVO_LIST_ID || process.env.BREVO_LIST_ID || "13", 10);
const brevoSenderEmail = import.meta.env.BREVO_SENDER_EMAIL || process.env.BREVO_SENDER_EMAIL;
const brevoSenderName =
  import.meta.env.BREVO_SENDER_NAME || process.env.BREVO_SENDER_NAME || "Dr Amira Medimagh, AWENE";
...
const cleanTextarea = (value, maxLength = 5000) =>
  String(value ?? "")
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, " ")
    .replace(/[<>]/g, "")
    .replace(/\r\n/g, "\n")
...
const cleanArray = (value, allowedValues) => {
  if (!Array.isArray(value)) {
    return [];
  }

  return [...new Set(value.map((entry) => cleanText(entry, 80)).filter((entry) => allowedValues.includes(entry)))];
};
...
const getGuidePdfUrl = (requestUrl) => {
  const configuredPdfUrl = import.meta.env.AWENE_GUIDE_PDF_URL || process.env.AWENE_GUIDE_PDF_URL;
...
const sanitizePayload = (raw) => ({
  ...
  most_appreciated: cleanTextarea(raw.most_appreciated, 3000),
  improvement_suggestions: cleanTextarea(raw.improvement_suggestions, 3000),
...
const validatePayload = (payload, rawInput) => {
  ...
  if (Array.isArray(rawInput.symptoms)) {
    const invalidSymptoms = rawInput.symptoms
      .map((value) => cleanText(value, 80))
      .filter((value) => !allowedValues.symptoms.includes(value));
...
export const POST = async ({ request, url }) => {
  ...
  // Remove full_name since the database table doesn't have this column
  ...
  try {
    ...
    return json({
      ok: true,
      message:
        "Je vous enverrai ensuite le guide récapitulatif de la masterclass. Pensez à vérifier votre dossier spam si vous ne le recevez pas."
    });
  } catch (error) {
