...
type ConvictionHighlight = {
  title: string;
  description: string;
  icon: "leaf" | "thermo" | "brain";
};
...
export type AboutPageContent = {
  ...
  beliefTitle: string;
  beliefParagraphs: readonly [string, string, string];
  beliefAnchor: string;
  quote: string;
  notSectionTitleLines: readonly [ReactNode, string, string, ReactNode];
  notSectionBullets: readonly [string, string, string];
  notSectionConclusion: string;
  companionEyebrow: string;
  companionTitle: string;
  companionCredentials: ReactNode;
  companionBio: readonly string[];
...
] as const satisfies readonly ConvictionHighlight[];
                    ...
                    {convictionHighlights.map((item, index) => (
                      <div key={item.title} className="flex items-start gap-4">
                        ...
                        <p className="text-base leading-relaxed md:text-[1.02rem]" style={{ color: "#5A5670", fontFamily: "var(--font-inter)" }}>
                          {content.beliefParagraphs[index]}
                        </p>
                      </div>
                    ))}
                  </div>
            ...
            <div className="relative py-4 md:py-6">
              ...
              <div className="relative grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.78fr)] lg:gap-14">
                <div>
                  ...
                  >
                    <span style={{ color: "#6F3FD6", fontStyle: "italic", fontWeight: 500, textTransform: "none" }}>{content.notSectionTitleLines[0]}</span>
                    {" "}
                    {content.notSectionTitleLines[1]}
                    <br />
                    {content.notSectionTitleLines[2]}
                    <br />
                    {content.notSectionTitleLines[3]}
                  </h2>
                </div>
                ...
                <div className="max-w-[30rem] lg:pt-3">
                  ...
                  >
                    {content.notSectionBullets.map((item) => (
                      <p key={item}>{item}</p>
                    ))}
                  </div>
                  <div
              ...
              <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(16rem,21rem)_minmax(0,1fr)] lg:items-stretch lg:gap-12">
                <div className="w-full max-w-[22rem] mx-auto lg:mx-0 lg:max-w-none">
                  <div className="aspect-[4/5] overflow-hidden rounded-[2rem] bg-[#F3ECFB] lg:h-full lg:aspect-auto">
                    <AmiraPortraitSlot alt={content.companionImageAlt} className="h-full w-full rounded-[2rem]" />
                  </div>
                </div>
                ...
                <div className="flex h-full flex-col">
                  ...
                  >
                    <span className="block h-px w-8 shrink-0" style={{ background: "#F68B2C" }} />
                    {content.companionEyebrow}
                  </p>
                  <h2 className={`${headingClass} mb-4`} style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>{content.companionTitle}</h2>
                  <p className="mb-7 text-base font-semibold leading-relaxed md:text-lg" style={{ color: "#F68B2C", fontFamily: "var(--font-inter)" }}>
                    {content.companionCredentials}
                  </p>
                  <div className="space-y-5" style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>
                    {content.companionBio.map((paragraph) => (
                      <p key={paragraph} className={copyClass}>{paragraph}</p>
                    ))}
                  </div>
                </div>
              </div>
                ...
                <div className="grid gap-8 lg:grid-cols-[minmax(14rem,18rem)_minmax(0,1fr)_minmax(0,1fr)] lg:items-start lg:gap-10">
                  <div className="transition-transform duration-300 group-hover:-translate-y-0.5">
                    <div className="relative aspect-[3.2/2.6] w-full overflow-hidden rounded-[1.5rem] shadow-[0_10px_24px_rgba(75,31,122,0.06)] transition-all duration-300 group-hover:shadow-[0_18px_34px_rgba(75,31,122,0.14)]">
                      <Image
                        src="/images/apropos-pourquoi-chemin-20260531.jpg"
                  ...
                  <div className="transition-transform duration-300 group-hover:-translate-y-0.5">
                    <h3 className="mb-4 text-2xl font-bold transition-colors duration-300 group-hover:text-[#6F3FD6]" style={{ fontFamily: "var(--font-playfair)", color: "#4B1F7A" }}>{content.pathTitle}</h3>
                    <p className="text-sm leading-relaxed md:text-[0.97rem]" style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>
                      {content.pathBody}
                    </p>
                  </div>
                  <div className="transition-transform duration-300 group-hover:-translate-y-0.5">
                    <h3 className="mb-4 text-2xl font-bold transition-colors duration-300 group-hover:text-[#6F3FD6]" style={{ fontFamily: "var(--font-playfair)", color: "#4B1F7A" }}>{content.distinctionTitle}</h3>
                    <p className="text-sm leading-relaxed md:text-[0.97rem]" style={{ color: "#6E6478", fontFamily: "var(--font-inter)" }}>
                      {content.distinctionBody}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Container>
            <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-12">
              <div className="lg:col-span-10 lg:col-start-2">
                <h2 className={`${headingClass} mb-8`} style={{ fontFamily: "var(--font-playfair)", color: "#2E2438" }}>{content.certificationsTitle}</h2>
